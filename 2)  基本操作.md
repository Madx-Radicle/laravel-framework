...
@csrf是(blade directive)[https://laravel.com/docs/8.x/blade#blade-directives]的一项指令。
...
除了检查POST的CSRF token, 中间件(middleware) App\Http\Middleware\VerifyCsrfToken也检查在请求X-CSRF-TOKEN

csrf token放在`<meta name="csrf-token" content="{{ csrf_token() }}">`使JavaScript更方便地获取到这个 CSRF 令牌，从而放置在请求的头信息中，例如:
```sh
$.ajaxSetup({
    headers: {
